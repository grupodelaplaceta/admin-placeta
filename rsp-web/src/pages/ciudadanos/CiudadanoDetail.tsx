...
export default function CiudadanoDetail() {
  ...
  async function guardarDatos() {
    if (!ctx) return;
    try {
      await provider.actualizarCiudadano(ctx.dip, { email, telefono });
  ...
  return (
    <>
      <PageHeader
        ...
        actions={<><Badge tone={ctx.nivel === 'N3' ? 'success' : 'info'}>Verificación {ctx.nivel}</Badge><Button size="sm" variant="outline" icon="download" onClick={descargarFicha}>Ficha PDF</Button></>}
      />
